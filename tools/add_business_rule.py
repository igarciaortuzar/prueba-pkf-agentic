...
Diseñado en ADR-006 (docs/adr/ADR-006-hook-proteccion-bash.md), sección 2.
`protect_bash_writes.py` reconoce la invocación de este script como una
excepción explícita al bloqueo general de escrituras vía Bash sobre
docs/business-rules.md — la validación de fondo (formato, numeración,
atomicidad) la hace este script, no el hook.
...
Claude Code (limitación reconocida en ADR-006).
...
def _atomic_write(content: str) -> None:
    ...
    escribir si algo falla a mitad de la escritura (ver ADR-006, seccion 2).
    """
...
def cmd_deprecate(args: argparse.Namespace) -> int:
    ...
    if not args.reemplazada_por:
        raise ValidationError(
            ...
            "'obsoleta' siempre registra qué RN la reemplaza (ver ADR-006)."
        )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Único camino sancionado para crear/obsoletar RN vía Bash (ADR-006)."
    )
